...
const updatePage = async () => {
  ...
  const whichElement = Math.floor(Math.random() * fruitsArray.length);
  const newElement = document.createElement('p');
  const order = fruitsArray[whichElement].order;
  const result = fruitsArray.filter(fruit => fruit.order == order);
  newElement.innerHTML = "Fruits of the order " + order + ": ";
  result.forEach(element => newElement.innerHTML = newElement.innerHTML + element.name + ', ');
